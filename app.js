(() => {
  ...
  document.addEventListener("click",e=>{
    const b=e.target.closest(".view-full-mob-stats");
    if(!b)return;
    const id=Number(b.dataset.mobStatId);
  ...
  function renderMobStatsDetail(id){
    ...
    const spawnRows=(m.spawns||[]).map((s,i)=>{
      ...
      const zone=cleanZoneName(s.zone); return `<div class="quest-req"><strong>${esc(zone||`Spawn ${i+1}`)}</strong><br>Respawn: ${timer}${coords}</div>`;
    }).join("");
