(() => {
  ...
  function renderMob(mob,targetId="detailPanel"){
    ...
    const zones=[...new Set([...(mob.zones||[]),...((ms?.spawns||[]).map(s=>s.zone).filter(Boolean))])];
    ...
    const lootHtml=ordered.length?ordered.map(slot=>`
      <div class="drop-group">
        <div class="drop-group-title">${esc(slot)} <span>${groups.get(slot).length}</span></div>
        <div class="link-list">${groups.get(slot).map(d=>`
          <button type="button" class="link-btn entity-link" data-kind="item" data-id="${d.item.id}">
      ...
      : `<div class="notice">No drops match the current filters.</div>`;
