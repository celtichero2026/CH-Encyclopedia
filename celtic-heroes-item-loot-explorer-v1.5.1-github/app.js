(() => {
  ...
  const zoneText=m=>(m.zones&&m.zones.length)?m.zones.join(", "):"Unknown zone";
  ...
  function itemMatches(i,f){
    ...
    if(f.zone && !i.mobs.some(mid=>(mobById.get(mid)?.zones||[]).includes(f.zone)))return false;
    return true;
  }
  function mobHasMatchingDrop(m,f){
    if(f.zone && !(m.zones||[]).includes(f.zone))return false;
  ...
  function runSearch(){
    ...
    if(f.type!=="items"&&out.length<180&&(!f.source||f.source==="mob")){
      for(const m of mobs){
        ...
        out.push({kind:"mob",id:m.id,name:m.name,sub:`Level ${m.level??"?"} • ${m.drops.length} drops • ${zoneText(m)}`});
        if(out.length>=180)break;
      }
    }
  ...
  function renderEDLGuide(targetId="guideDetail"){
    ...
    const colorRows=Object.entries(q.classColors||{}).map(([cls,color])=>
      `<div class="stat-row"><span>${esc(cls)}</span><strong>${esc(color)}</strong></div>`).join("");
  ...
  function renderMob(mob,targetId="detailPanel"){
    if(!mob)return;const f=currentFilters();
    ...
    for(const d of drops){const slot=d.item.stats?.slot||"Other";if(!groups.has(slot))groups.set(slot,[]);groups.get(slot).push(d)}
    for(const arr of groups.values())arr.sort((a,b)=>((a.item.rarity||"").localeCompare(b.item.rarity||""))||a.item.name.localeCompare(b.item.name));
    const ordered=[...groups.keys()].sort((a,b)=>(slotOrder.indexOf(a)<0?99:slotOrder.indexOf(a))-(slotOrder.indexOf(b)<0?99:slotOrder.indexOf(b))||a.localeCompare(b));
    $(targetId).innerHTML=`${targetId==="guideDetail"&&activeGuideId?`<button type="button" class="clear-btn back-guide-btn" data-guide="${activeGuideId}">← Back to guide</button>`:""}
      <div class="detail-heading-row">
        <div><div class="detail-kicker">Mob</div><h2 class="detail-title">${esc(mob.name)}</h2></div>
        ${favoriteButton("mob",mob.id)}
      </div>
      <div class="meta"><span>Level ${mob.level??"?"}</span><span>${mob.stars??"?"}★</span><span>${drops.length} visible drops</span>${mob.zones?.length?`<span>${esc(mob.zones.join(", "))}</span>`:""}</div>
      <div class="section-title">Loot</div>
      ${ordered.map(slot=>`<div class="loot-group"><div class="loot-group-head"><span>${esc(slot)}</span><span class="loot-group-count">${groups.get(slot).length}</span></div><div class="link-list">${groups.get(slot).map(d=>`<button type="button" class="link-btn entity-link" data-kind="item" data-id="${d.item.id}"><div class="link-main"><span>${esc(d.item.name)}</span><span>${d.item.rarity?`<span class="badge rarity">${esc(d.item.rarity)}</span>`:""}</span></div><div class="link-sub">${[d.item.stats?.classReq,d.item.stats?.levelReq?`Lv ${d.item.stats.levelReq}`:null].filter(Boolean).join(" • ")||"Unrestricted"}</div></button>`).join("")}</div></div>`).join("")}
      ${!drops.length?`<div class="notice">No drops match the current class / slot / level filters.</div>`:""}
      <div class="notice">Obvious internal/test items and mobs are hidden. Loot-table values are not shown as final percentages because the source tables may be nested.</div>`;
  }
  $("detailPanel").addEventListener("click",e=>{const b=e.target.closest(".entity-link");if(b)openEntity(b.dataset.kind,Number(b.dataset.id))});


  ["detailPanel","guideDetail","favoritesDetail"].forEach(id=>{
    $(id).addEventListener("click",e=>{
      const fav=e.target.closest(".favorite-btn");
      if(!fav)return;
      e.stopPropagation();
      toggleFavorite(fav.dataset.favoriteKind,Number(fav.dataset.favoriteId));
    });
  });

  $("guideDetail").addEventListener("click",e=>{
    const back=e.target.closest(".back-guide-btn");
    if(back){openGuide(back.dataset.guide);return;}
    const b=e.target.closest(".entity-link");
    if(!b)return;
    openEntity(b.dataset.kind,Number(b.dataset.id),"guideDetail");
  });
  ...
  function renderFavorites(){
    ...
    for(const key of favorites){
      ...
      if(kind==="item"){
        ...
        if(m) rows.push({kind,id,name:m.name,sub:`Level ${m.level??"?"} • ${zoneText(m)}`});
      }
    }
